...
class bondpad(DloGen):
    ...
    def defineParamSpecs(self, specs):
        ...
        shape       = techparams['bondpad_shape']
        stack       = techparams['bondpad_stack']

        specs('cdf_version', CDFVersion, 'CDF Version')
        specs('Display', 'Selected', 'Display', ChoiceConstraint(['All', 'Selected']))
        specs('model', model, 'Model name')

        specs('shape', shape, 'Shape', ChoiceConstraint(['octagon', 'square', 'circle']))
        specs('stack', stack, 'Stack Metals', ChoiceConstraint(['nil', 't']))
        ...
        specs('bottomMetal', bottomMetal, 'BottomMetal', ChoiceConstraint(['4', '5', 'TM1']))
    ...
    def genLayout(self):
        ...
        V1_size       = techparams['V1_a']      # read for contract parity; inert (metal==1 unreachable)
        V1_dist       = techparams['V1_b']      # idem
        ...
        metallization = techparams['metalName']  # read only; drives nothing
        ...
        # sg13 metal mapping (hardcoded; not derived from metalName)
        if topMetal == 'TM1':
            topMetal = 6
        else:
            topMetal = 7
        ...
        # interposer floor is the fourth metal, not the base-layer floor
        if bottomMetal < 4:
            bottomMetal = 4
        ...
        # None-padded so indices stay identical to upstream; indices 0..2 are
        # never dereferenced because bottomMetal is clamped >= 4.
        drawMetalList = [None, None, None, 'Metal4', 'Metal5', 'TopMetal1', 'TopMetal2']
        drawViaList   = [None, None, None, 'Via4',   'TopVia1',  'TopVia2']
        tm1ind = 6
        tm2ind = 7 if topMetal == 7 else 0
        noFillerStack = drawMetalList[3:topMetal]
        ...
        if shape == 'octagon':
            ...
            if stack:
                ...
                for metal in range(int(bottomMetal), int(metalNumber)):
                    ...
                    if metal == 1:
                        vs = V1_size; vd = V1_dist
                    if metal > 1 and metal < tm1ind-1:
                        vs = Vn_size; vd = Vn_dist
                    if metal == tm1ind-1:
                        vs = TV1_size; vd = TV1_dist
                    if metal == tm1ind:
                        vs = TV2_size; vd = TV2_dist
            ...
            if shape == 'square':
                ...
                if addFillerEx:
                    oradx = radx+noFillerEnc
                    orady = rady+noFillerEnc
                    # Draw each exclusion circle directly on its layer. The
                    # upstream dbCopyShape(...)+id.layer= retag is a no-op under
                    # pycell4klayout-api (the same idiom the CuPillarPad file
                    # avoids), which would silently emit only the first nofill.
                    for noFiller in noFillerStack:
                        bondpadStretchedCircle(self, Layer(noFiller, 'nofill'), oradx, orady, grid)

                metalNumber = topMetal
                pcLayer = nth(metalNumber-1, drawMetalList)
                bondpadStretchedCircle(self, pcLayer, radx, rady, grid)

                # dfpad drawn directly on its layer (retag idiom is inert here).
                if padType == 'bondpad':
                    bondpadStretchedCircle(self, padLay, radx, rady, grid)
                ...
                if stack:
                    for metal in range(bottomMetal, metalNumber):
                        ...
                        oradx = radx-stripeWidth
                        orady = rady-stripeWidth
                        # Outer and inner discs both on pcLayer, XORed into a
                        # ring (matching the octagon/square intermediate-metal
                        # rings). Drawn directly instead of copy-retagged so the
                        # metal actually lands on its own layer.
                        id1 = bondpadStretchedCircle(self, pcLayer, radx, rady, grid)
